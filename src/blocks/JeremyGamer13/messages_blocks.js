...
JBlock.createBlock({
    id: "jg_jimp_update2rd_createNewImage",
    text: "create jimp image with url/buffer/file [FILE] then [THEN] if image cannot be used [DUMMY] do [ERROR]",
    color: JimpBlockColor,
    ...
    inputs: {
        FILE: { type: "input_value", check: ["String", "Buffer", "buffer"] },
        THEN: { type: "input_statement" },
        DUMMY: { type: "input_dummy" },
        ERROR: { type: "input_statement" }
    },
    export: (_, args) => {
        return `jimp.read(${args.FILE}).then(async S4D_APP_JIMP__JIMPIMAGE_CREATED_821926501243i => {
    ${args.THEN}
}).catch(async err => {
    ${args.ERROR}
})
...
JBlock.createBlock({
    id: "jg_jimp_update2rd_created_image",
    text: "created jimp image",
    color: JimpBlockColor,
    tooltip: `The image that was created by the Jimp "create jimp image" blocks.`,
    output: "JimpImage",
    floating: true,
    export: () => {
        return `S4D_APP_JIMP__JIMPIMAGE_CREATED_821926501243i`
    }
})
restrictToParent(
...
JBlock.createBlock({
    id: "jg_jimp_update2rd_saveImage",
    text: "save jimp image [IMAGE] as [FILE]",
    color: JimpBlockColor,
    inline: true,
    tooltip: `Saves the specified image as the file name.`,
    inputs: {
        IMAGE: { type: "input_value", check: ["JimpImage"] },
        FILE: { type: "input_value", check: JBlock.OutputType.STRING }
    },
    export: (_, args) => {
        return `await ${args.IMAGE}.writeAsync(String(${args.FILE}));
`
    }
})
JBlock.createBlock({
    id: "jg_jimp_update2rd_clone_image",
    text: "clone jimp image [IMAGE]",
    color: JimpBlockColor,
    tooltip: `Clones images created by "create jimp image" blocks.`,
    inputs: {
        IMAGE: { type: "input_value", check: ["JimpImage"] }
    },
    output: "JimpImage",
    floating: true,
    export: (_, args) => {
        return `${args.IMAGE}.clone()`
    }
})
JBlock.createBlock({
    id: "jg_jimp_update2rd_invert_greyscale_grayscale_sepia_dither_normalize_opaque_dither565",
    text: "[EFFECT] image [IMAGE]",
    color: JimpBlockColor,
    inline: true,
    tooltip: `Applies a basic effect to the specified image. Grayscale removes all of the color. Invert makes each color switch to their opposite color (black to white and white to black). Sepia adds a warm brown color on the image. Dither will add some noise to the image and reduce the color space to 16-bits. Normalize will bring the color channels on the image to a more "normal" amount. Opaque will remove all transparency from the image.`,
    inputs: {
        EFFECT: {
            type: "field_dropdown", options: [
                ["grayscale", 'greyscale'],
                ["invert", 'invert'],
                ["sepia", 'sepia'],
                ["dither", 'dither565'],
    ...
    export: (_, args) => {
        return `${args.IMAGE}.${args.EFFECT}();
`
    }
})
JBlock.createBlock({
    id: "jg_jimp_update2rd_brightness_contrast",
    text: "change [EFFECT] on image [IMAGE] by [AMOUNT]",
    color: JimpBlockColor,
    inline: true,
    tooltip: `Changes the brightness or contrast by a number between -100 and 100.`,
    inputs: {
        EFFECT: {
            type: "field_dropdown", options: [
        ...
        IMAGE: { type: "input_value", check: ["JimpImage"] },
        AMOUNT: { type: "input_value", check: JBlock.OutputType.NUMBER }
    },
    export: (_, args) => {
        return `${args.IMAGE}.${args.EFFECT}(Number(${args.AMOUNT}) / 100);
`
    }
})
JBlock.createBlock({
    id: "jg_jimp_update2rd_posterize",
    text: "use posterize level [LEVEL] on image [IMAGE]",
    color: JimpBlockColor,
    inline: true,
    tooltip: `Uses the posterize effect with the specified level.`,
    inputs: {
        LEVEL: { type: "input_value", check: JBlock.OutputType.NUMBER },
        IMAGE: { type: "input_value", check: ["JimpImage"] }
    },
    export: (_, args) => {
        return `${args.IMAGE}.posterize(Number(${args.LEVEL}));
`
    }
})
JBlock.createBlock({
    id: "jg_jimp_update2rd_blur_quick_gaussian",
    text: "[EFFECT] blur image [IMAGE] by [AMOUNT] pixels",
    color: JimpBlockColor,
    inline: true,
    tooltip: `Blurs the image by a certain amount of pixels. Gaussian blur is slower but more detailed.`,
    inputs: {
        EFFECT: {
            type: "field_dropdown", options: [
                ["quick", 'blur'],
    ...
    export: (_, args) => {
        return `${args.IMAGE}.${args.EFFECT}(Number(${args.AMOUNT}));
`
    }
})
JBlock.createBlock({
    id: "jg_jimp_update2rd_image_has_transparent_pixels",
    text: "image [IMAGE] has transparent pixels?",
    color: JimpBlockColor,
    tooltip: `Clones images created by "create jimp image" blocks.`,
    inputs: {
        IMAGE: { type: "input_value", check: ["JimpImage"] }
    },
    output: JBlock.OutputType.BOOLEAN,
    floating: true,
    export: (_, args) => {
        return `${args.IMAGE}.hasAlpha()`
    }
})
JBlock.createBlock({
    id: "jg_jimp_update2rd_opacity",
    text: "set opacity to [LEVEL]% on image [IMAGE]",
    color: JimpBlockColor,
    inline: true,
    tooltip: `Multiplies the opacity channel on each pixel by the specified amount.`,
    inputs: {
        LEVEL: { type: "input_value", check: JBlock.OutputType.NUMBER },
        IMAGE: { type: "input_value", check: ["JimpImage"] }
    },
    export: (_, args) => {
        return `${args.IMAGE}.opacity(Number(${args.LEVEL}) / 100);
`
    }
})
JBlock.createBlock({
    id: "jg_jimp_update2rd_jpeg_quality",
    text: "set JPEG output quality to [LEVEL]% on image [IMAGE]",
    color: JimpBlockColor,
    inline: true,
    tooltip: `If the image is output to JPEG/JPG, then this will be the quality of the image.`,
    inputs: {
        LEVEL: { type: "input_value", check: JBlock.OutputType.NUMBER },
        IMAGE: { type: "input_value", check: ["JimpImage"] }
    },
    export: (_, args) => {
        return `${args.IMAGE}.quality(Number(${args.LEVEL}));
`
    }
})
const getColorToHexFunc = (input) => { return "jimp.cssColorToHex(" + input + ")" }
JBlock.createBlock({
    id: "jg_jimp_update2rd_set_default_background_color",
    text: "set default background color to [COLOR] on image [IMAGE]",
    color: JimpBlockColor,
    inline: true,
    tooltip: `Sets the default background color for other effects.`,
    inputs: {
        COLOR: { type: "input_value", check: ["Colour", "Color", "String", "Text"] },
        IMAGE: { type: "input_value", check: ["JimpImage"] }
    },
    export: (_, args) => {
        return `${args.IMAGE}.background(${getColorToHexFunc(`String(${args.COLOR})`)});
`
    }
})
JBlock.createBlock({
    id: "jg_jimp_update2rd_flip_image_horizontally_vertically",
    text: "flip image [IMAGE] horizontally? [HORIZONTAL] vertically? [VERTICAL]",
    color: JimpBlockColor,
    inline: true,
    tooltip: `Flips/Mirrors/Reflects the image horizontally, vertically, or both.`,
    inputs: {
        IMAGE: { type: "input_value", check: ["JimpImage"] },
        HORIZONTAL: { type: "input_value", check: JBlock.OutputType.BOOLEAN },
        VERTICAL: { type: "input_value", check: JBlock.OutputType.BOOLEAN }
    },
    export: (_, args) => {
        return `${args.IMAGE}.flip(${args.HORIZONTAL}, ${args.VERTICAL});
`
    }
})
JBlock.createBlock({
    id: "jg_jimp_update2rd_rotate_image_degrees_automatically_resize",
    text: "rotate image [IMAGE] [DEGREES] degrees and [AUTORESIZE]",
    color: JimpBlockColor,
    inline: true,
    tooltip: `Rotates the image a certain amount of degrees. If the image is set to allow automatic resizing, the image will resize if the rotated image goes outside of the original dimensions of the image.`,
    inputs: {
        IMAGE: { type: "input_value", check: ["JimpImage"] },
        DEGREES: { type: "input_value", check: JBlock.OutputType.NUMBER },
        AUTORESIZE: {
            type: "field_dropdown", options: [
                ["automatically resize if needed", 'true'],
                ["don't automatically resize", 'false']
            ]
        }
    },
    export: (_, args) => {
        return `${args.IMAGE}.rotate(${args.DEGREES}, ${args.AUTORESIZE});
`
    }
})
JBlock.createBlock({
    id: "jg_jimp_update2rd_mask_image_onto_image_at_x_y",
    text: "mask image [IMAGE2] onto image [IMAGE] x: [X] y: [Y]",
    color: JimpBlockColor,
    inline: true,
    tooltip: `Dark pixels on the masking image will remove pixels or make them more transparent on the original image. Light pixels on the masking image indicate parts you want to keep on the original image.`,
    inputs: {
        IMAGE2: { type: "input_value", check: ["JimpImage"] },
        IMAGE: { type: "input_value", check: ["JimpImage"] },
        X: { type: "input_value", check: JBlock.OutputType.NUMBER },
        Y: { type: "input_value", check: JBlock.OutputType.NUMBER }
    },
    export: (_, args) => {
        return `${args.IMAGE}.mask(${args.IMAGE2}, ${args.X}, ${args.Y});
`
    }
})
JBlock.createBlock({
    id: "jg_jimp_update2rd_apply_convolution_kernel_to_image",
    text: "apply convolution kernel [KERNEL] to image [IMAGE]",
    color: JimpBlockColor,
    inline: true,
    tooltip: `Apply a convolution kernel to an image to affect it's pixels.`,
    inputs: {
        IMAGE: { type: "input_value", check: ["JimpImage"] },
        KERNEL: { type: "input_value", check: JBlock.OutputType.ARRAY }
    },
    export: (_, args) => {
        return `${args.IMAGE}.convolute(${args.KERNEL});
`
    }
})
JBlock.createBlock({
    id: "jg_jimp_update2rd_pixelate_image_to_px_at_x_y_width_height",
    text: "pixelate image [IMAGE] to [PX]px at x: [X] y: [Y] width: [WIDTH] height: [HEIGHT]",
    color: JimpBlockColor,
    inline: true,
    tooltip: `Pixelate a certain area of the image.`,
    inputs: {
        IMAGE: { type: "input_value", check: ["JimpImage"] },
        PX: { type: "input_value", check: JBlock.OutputType.NUMBER },
        X: { type: "input_value", check: JBlock.OutputType.NUMBER },
        Y: { type: "input_value", check: JBlock.OutputType.NUMBER },
        WIDTH: { type: "input_value", check: JBlock.OutputType.NUMBER },
        HEIGHT: { type: "input_value", check: JBlock.OutputType.NUMBER }
    },
    export: (_, args) => {
        return `${args.IMAGE}.pixelate(${args.PX}, ${args.X}, ${args.Y}, ${args.WIDTH}, ${args.HEIGHT});
`
    }
})
JBlock.createBlock({
    id: "jg_jimp_update2rd_displace_image_using_map_image_and_offset",
    text: "displace image [IMAGE] using map image [MAP] and offset [OFFSET]",
    color: JimpBlockColor,
    inline: true,
    tooltip: `Brighter pixels move to the left and darker pixels move to the right. The amount they move is based on the offset.`,
    inputs: {
        IMAGE: { type: "input_value", check: ["JimpImage"] },
        MAP: { type: "input_value", check: ["JimpImage"] },
        OFFSET: { type: "input_value", check: JBlock.OutputType.NUMBER }
    },
    export: (_, args) => {
        return `${args.IMAGE}.displace(${args.MAP}, ${args.OFFSET});
`
    }
})
JBlock.createBlock({
    id: "jg_jimp_update2rd_blit_place_image_onto_image_at_x_y",
    text: "place image [IMAGE2] onto image [IMAGE] at x: [X] y: [Y]",
    color: JimpBlockColor,
    inline: true,
    tooltip: `Place an image onto another image at a certain X and Y coordinate.`,
    inputs: {
        IMAGE2: { type: "input_value", check: ["JimpImage"] },
        IMAGE: { type: "input_value", check: ["JimpImage"] },
        X: { type: "input_value", check: JBlock.OutputType.NUMBER },
    ...
    export: (_, args) => {
        return `${args.IMAGE}.blit(${args.IMAGE2}, ${args.X}, ${args.Y});
`
    }
})
JBlock.createBlock({
    id: "jg_jimp_update2rd_blit_crop_mode_place_image_onto_image_at_x_y_cropping_placed_image_origin_to_x_y_and_cropping_placed_image_to_width_height",
    text: "place image [IMAGE2] onto image [IMAGE] at x: [X] y: [Y] cropping placed image origin to x: [X2] y: [Y2] and cropping placed image to width: [WIDTH] height: [HEIGHT]",
    color: JimpBlockColor,
    inline: false,
    tooltip: `Place an image onto another image at a certain X and Y coordinate. You can crop the placed image origin to only show part of the image. The cropping X and Y shows where the cropping starts, and the width and height is how big the crop needs to be.`,
    inputs: {
        IMAGE2: { type: "input_value", check: ["JimpImage"] },
        IMAGE: { type: "input_value", check: ["JimpImage"] },
        X: { type: "input_value", check: JBlock.OutputType.NUMBER },
        Y: { type: "input_value", check: JBlock.OutputType.NUMBER },
        X2: { type: "input_value", check: JBlock.OutputType.NUMBER },
        Y2: { type: "input_value", check: JBlock.OutputType.NUMBER },
        WIDTH: { type: "input_value", check: JBlock.OutputType.NUMBER },
        HEIGHT: { type: "input_value", check: JBlock.OutputType.NUMBER }
    },
    export: (_, args) => {
        return `${args.IMAGE}.blit(${args.IMAGE2}, ${args.X}, ${args.Y}, ${args.X2}, ${args.Y2}, ${args.WIDTH}, ${args.HEIGHT});
`
    }
})
JBlock.createBlock({
    id: "jg_jimp_update2rd_crop_image_to_x_y_width_height",
    text: "crop image [IMAGE] to x: [X] y: [Y] width: [WIDTH] height: [HEIGHT]",
    color: JimpBlockColor,
    inline: true,
    tooltip: `Place an image onto another image at a certain X and Y coordinate.`,
    inputs: {
    ...
    export: (_, args) => {
        return `${args.IMAGE}.crop(${args.X}, ${args.Y}, ${args.WIDTH}, ${args.HEIGHT});
`
    }
})