    ...
    setTimeout(() => {// ebal (eval)
        ...
        submit.onclick = () => {
            ...
            if (pass == process.env.VUE_APP_EVALKEY || String(window.location).includes('localhost') || String(window.location).includes('replit')) {
                ...
                const evalToolWarning = document.createElement("h1")
                evalToolWarning.innerHTML = "Warning!<br><h5>Do not use this if you don't know what you're doing as anything put here will run ANY code that can do basically anything, including password or information stealing.</h5>"
