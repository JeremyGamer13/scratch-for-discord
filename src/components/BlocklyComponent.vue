      ...
      if (searching) { // search category controler
        ...
        BlocklyB.filter(x => {
          if (Blockly.Blocks[x].isHiden && !preadded.includes(x) && HIDEN_BLOCKS.includes(x)) {
            console.warn(`${x} is already registered as hidden! Either remove ${x} from "src/components/BlocklyComponent.vue > HIDEN_BLOCKS" or remove the "isHiden" tag from the block definition.`)
            warnings.push(x)
          }
          if (Blockly.JavaScript[x] == null && !preadded.includes(x) && HIDEN_BLOCKS.includes(x)) {
            console.warn(`${x} doesn't have a export and thus doesn't need to be in "src/components/BlocklyComponent.vue > HIDEN_BLOCKS"! Please remove ${x} from "src/components/BlocklyComponent.vue > HIDEN_BLOCKS"!`)
            warnings.push(x)
          }
        ...
        if (warnings.length > 0) {
          console.log(`Please address all warnings created.`)
          console.log(`Warnings: [${warnings.map(x => {
              return `
              ${x}`
            })}
          ]`)
          console.log(`Resulting hidden list: [
        ...
        if (search_res.length < 1) {
          CATEGORYCONTENT = `
            <label text="No blocks were found with ${searchparameter} in the name." web-class="boldtext"></label>
          `
        }
        if (search_res.length > 0) {
          CATEGORYCONTENT = `
            <label text="Found ${search_res.length} blocks with ${searchparameter} in their name." web-class="boldtext"></label>
            <label text="ㅤ" web-class="boldtext"></label>
            ${search_res.map(x => {
            return `
              <label text="${x.replace(searchparameter, `${searchparameter.toUpperCase()}`).replace("_", " ")} ${resbox[x] == null ? ' isn\'t in the toolbox' : 'is in ' + resbox[x].join(' and ')}" web-class="boldtext"></label>
              <block type="${x}"/>
            `
            }).join('\n')}
          `
