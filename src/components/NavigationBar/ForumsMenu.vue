<template>
    <b-nav-item @click="forums">Forums
        <i class="fa-solid fa-circle-dot" id="forums_notification_indicator_wakeupmf_tyougotanotif_brh"
            style="display:none">
            <div
                style="position:absolute;background-color:red;width: 50%;height: 50%;left: 29%;top:15%;border-radius: 50%;color:red">
            </div>
            <div id="forums_notification_indicator_wakeupmf_tyougotanotif_brh_MMMMMMM"
                style="position:absolute;color:white;left: 22%;top:6%;font-size:smaller">1</div>
    ...
    */
    ...
    if (matches) {
        matches.forEach(match => {
            newString = newString.replace(match, '<img alt="Icon for this Website" src="https://t2.gstatic.com/faviconV2?client=SOCIAL&type=FAVICON&fallback_opts=TYPE,SIZE,URL&url=' + encodeURIComponent(match) + '&size=16"> <a target="_blank" href="https://s4djt-forums-beta.jeremygamer13.repl.co/forums/getUrlRedirectPage/?url=' + Buffer.from(match, 'UTF8').toString('Hex') + '">' + match.replace(/(http(s|):\/\/|\/\S*)/gmi, "") + '</a>')
        })
    }
...
function loadForumsPage(page, div) {
    ...
    switch (page) {
        case 'register':
            div.innerHTML = `
<br>
<h3>Create an S4D JT Forums Beta account</h3>
...
<br>
<br>
<p style="color:rgb(100,100,100)"><i>Note: These forum accounts are temporary. The account system will be changed in the future.</i></p>
`
            document.getElementById("s4d_forums_register_button").onclick = () => {
                ...
                fetch(`https://s4djt-forums-beta.jeremygamer13.repl.co/register`, requestOptions).then(result => result.json().then(json => {
        ...
        case 'login':
            div.innerHTML = `
<br>
<h3>Login to your S4D JT Forums Beta account</h3>
<i class="fa-solid fa-user"></i> <input type="text" id="s4d_forums_username_input" placeholder="Username">
<br>
<i class="fa-solid fa-key"></i> <input type="password" id="s4d_forums_password_input" placeholder="Password">
<br>
...
`
            document.getElementById("s4d_forums_login_button").onclick = () => {
                ...
                fetch(`https://s4djt-forums-beta.jeremygamer13.repl.co/validateLogin`, requestOptions).then(result => result.json().then(json => {
                    if (!json.success) {
                        displaySpinner.innerHTML = `<i class="fa-solid fa-circle-xmark"></i> &#8226 ${json.error}`
                        displaySpinner.setAttribute("style", "color:#ff0000")
                        return
...
`
                    ...
                    localforage.getItem("FORUMS_PASSWORD").then((password) => {
                        localforage.getItem("FORUMS_CATEGORY").then(name => {
                            ...
                            setTimeout(() => {
                                ...
                                fetch(`https://s4djt-forums-beta.jeremygamer13.repl.co/forums/post`, requestOptions).then(result => result.json().then(json => {
                                    if (result.status != 200) {
            ...
            localforage.getItem("FORUMS_CATEGORY").then(name => {
                ...
                fetch(`https://s4djt-forums-beta.jeremygamer13.repl.co/forums/getCategory?category=${name}`).then(result => result.json().then(json => {
                    if (result.status != 200) {
                    ...
                    json.forEach(post => {
                        ...
                        forumPost.innerHTML = `<div class="forums-post-author"><p id="forums_post_${post.id}_author_name">${post.author}</p> &#8226 <div class="forums-post-author-image" id="forums_post_${post.id}_author_imageContainer"><img src="https://s4djt-forums-beta.jeremygamer13.repl.co/getFallbackIcon" width="32" height="32" alt="${post.author}"></div></div>
...
<small><i class="fa-solid fa-user-pen"></i> ${(post.comments || []).length}, <i class="fa-solid fa-message"></i> ${(post.subposts || []).length}</small>`
                    ...
                    postsToBeLoaded.forEach(post => {
                        ...
                        fetch(`https://s4djt-forums-beta.jeremygamer13.repl.co/accounts/getUser?username=${encodeURIComponent(post.author)}`).then(result => result.json().then(json => {
                            let keepGoing = true
                            if (result.status != 200) {
                                usernameArea.innerHTML = "Unknown Account"
                                imageArea.getElementsByTagName("img").item(0).src = `https://s4djt-forums-beta.jeremygamer13.repl.co/getFallbackIcon`
                                keepGoing = false
                            }
                            if (keepGoing) {
        ...
        <div class="forums-post-subposts">
            ...
            <div class="forums-post-author-information">
                <div>
                    ...
                    <img id="s4d_forums_post_author_image" width="96" height="96" src="https://s4djt-forums-beta.jeremygamer13.repl.co/getFallbackIcon">
                </div>
            </div>
                ...
                localforage.getItem("FORUMS_PASSWORD").then(password => {
                    ...
                    localforage.getItem("FORUMS_CATEGORY").then(category => {
                        ...
                        localforage.getItem("FORUMS_POST_ID").then(postid => {
                            ...
                            fetch(`https://s4djt-forums-beta.jeremygamer13.repl.co/forums/getPost?category=${category}&postid=${postid}`).then(result => result.json().then(json => {
...
`
                                ...
                                fetch(`https://s4djt-forums-beta.jeremygamer13.repl.co/accounts/getUser?username=${post.author}`).then(result => result.json().then(json => {
                                    if (result.status != 200) {
                                        document.getElementById("s4d_forums_post_author_name").innerHTML = "Unknown Poster"
                                        document.getElementById("s4d_forums_post_author_image").src = "https://s4djt-forums-beta.jeremygamer13.repl.co/getFallbackIcon"
                                        return
                                    }
                                ...
                                post.comments.forEach(comment => {
                                    ...
                                    if (username == comment.author) {
                                        ...
                                        fetch(`https://s4djt-forums-beta.jeremygamer13.repl.co/accounts/getUser?username=${username}`).then(result => result.json().then(json => {
                                    ...
                                    trashIcon.onclick = () => {
                                        ...
                                        if (usurebro) {
                                            ...
                                            fetch(`https://s4djt-forums-beta.jeremygamer13.repl.co/forums/deleteCommentOnPost`, requestOptions).then(result => result.json().then(json => {
                                                if (!json.error) {
                                                    loadForumsPage("forumPost", div)
                                                    return
                                                }
...
> ${post.title}`)
                                ...
                                if (editTitleButton) {
                                    ...
                                    submitButton.onclick = () => {
                                        ...
                                        fetch(`https://s4djt-forums-beta.jeremygamer13.repl.co/forums/editPost`, requestOptions).then(result => result.json().then(json => {
                                            if (!json.error) {
                                                loadForumsPage("forumPost", div)
                                                return
                                            }
                                ...
                                if (post.author == username) {
                                    ...
                                    fetch(`https://s4djt-forums-beta.jeremygamer13.repl.co/accounts/getUser?username=${encodeURIComponent(username)}`).then(result => result.json().then(json => {
                                        if (json.moderator == true) {
                                            document.getElementById("s4d_forums_ifCanDeletePostThen_showthisDiv").setAttribute("style", "color:#ff0000")
                                        }
                                    }))
                                ...
                                deletePostButton.onclick = () => {
                                    ...
                                    fetch(`https://s4djt-forums-beta.jeremygamer13.repl.co/forums/deletePost`, requestOptions).then(result => result.json().then(json => {
                                        if (!json.error) {
                                            loadForumsPage("forumCategory", div)
                                            return
                                        }
                                ...
                                postCommentButton.onclick = () => {
                                    ...
                                    fetch(`https://s4djt-forums-beta.jeremygamer13.repl.co/forums/commentOnPost`, requestOptions).then(result => result.json().then(json => {
                                        if (!json.error) {
                                            loadForumsPage("forumPost", div)
                                            return
                                        }
            ...
            localforage.getItem("FORUMS_USERNAME").then(username => {
                ...
                localforage.getItem("FORUMS_PASSWORD").then(password => {
                    ...
                    fetch(`https://s4djt-forums-beta.jeremygamer13.repl.co/accounts/getUser?username=${username}`).then(result => result.json().then(json => {
                        ...
                        imageArea.src = (json.custompfp ? json.custompfp : "https://s4djt-forums-beta.jeremygamer13.repl.co/getFallbackIcon")
                        ...
                        if (json.moderator == true) {
                            ...
                            imageArea.onclick = () => {
                                ...
                                fetch(`https://s4djt-forums-beta.jeremygamer13.repl.co/accounts/setUserCustomPFP`, requestOptions).then(result => result.json().then(json => {
                                    if (json.error) {
                                        alert(json.error)
                                        return
                                    }
                    ...
                    fetch(`https://s4djt-forums-beta.jeremygamer13.repl.co/getAllColors`).then(result => result.json().then(json => {
                        ...
                        colorSelection.onchange = (event) => {
                            ...
                            fetch(`https://s4djt-forums-beta.jeremygamer13.repl.co/accounts/setUserColor`, requestOptions).then(result => result.json().then(json => {
                                if (json.error) {
                                    alert(json.error)
                                    return
                                }
                    ...
                    document.getElementById("s4d_forums_accountMenu_profilePicture_displayName_update").onclick = () => {
                        ...
                        fetch(`https://s4djt-forums-beta.jeremygamer13.repl.co/accounts/setUserDisplayName`, requestOptions).then(result => result.json().then(json => {
                    ...
                    document.getElementById("s4d_forums_accountMenu_profilePicture_updatePassword_submitButton").onclick = () => {
                        ...
                        fetch(`https://s4djt-forums-beta.jeremygamer13.repl.co/accounts/setUserNewPassword`, requestOptions).then(result => result.json().then(json => {
...
<p id="s4d_forums_invalid_page_return" class="forums-fakeUrl">Return to Register Page</p>`
    ...
    readButton.onclick = () => {
        localforage.getItem("FORUMS_USERNAME").then(username => {
            localforage.getItem("FORUMS_PASSWORD").then(password => {
                ...
                fetch(`https://s4djt-forums-beta.jeremygamer13.repl.co/accounts/clearNotifications`, requestOptions).then(result => result.json().then(json => {
                    if (json.error) {
                        alert(json.error)
                    }
    ...
    localforage.getItem("FORUMS_USERNAME").then(username => {
        ...
        fetch(`https://s4djt-forums-beta.jeremygamer13.repl.co/accounts/getNotifications`, requestOptions).then(result => result.json().then(json => {
                ...
                localforage.setItem("FORUMS_PAGE", "forumPost").then(async () => {
                    localforage.setItem("FORUMS_CATEGORY", urlParams.get("forumcategory")).then(async () => {
                        localforage.setItem("FORUMS_POST_ID", urlParams.get("forumpost")).then(async () => {
                            ...
                            warning.innerHTML = `<i style="margin-left: 0.5%" class="fa-solid fa-triangle-exclamation"></i> S4D JT Forums are still work in progress. Don't hesitate to give feedback in the official Discord server.`
